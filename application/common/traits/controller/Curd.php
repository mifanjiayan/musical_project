...
trait Curd
{
    ...
    public function __construct(Request $request)
    {
        ...
        $this->imgPre = config('setting.img_prefix');
...
//        }
    ...
    public function save()
    {
        ...
        $data = request()->post();
        Log::info('添加数据 = '.json_encode($data));
                ...
                if (empty($this->translations[$v]['type'])) {
                    ...
                    switch ($this->translations[$v]['type']) {
                        ...
                        case 'join':
                            ...
                            if(!empty($this->translations[$v]['data']['type'])){
                                $table .= '<img src="'.$this->imgPre.$value[$show_field].'" style="width: 300px; height: 80px;">';
                            }else{
...
//                    }
    ...
    public function getSearchHtml()
    {
        ...
        if (!empty($this->searchFields)) {
            $html .= '<form role="form" class="form-inline" id="form" style="margin-left: -8px;">';
            //常用搜索字段
            $html .= $this->searchField($this->searchFields);
            $html .= '<div class="btn btn-default btn-outline btn-link" style="margin-left: 10px; margin-top: 5px;" id="more_search">更多条件</div>';
            //不常用搜索字段
            ...
            $html .= '<div class="form-group" style="margin-left: 10px; margin-top: 5px;"><input type="button"  class="btn btn-primary" id="search" value="搜索">&nbsp;&nbsp;&nbsp;<a class="btn btn-default btn-outline" href="' . url($this->route . '/index') . '">重置</a></div>';
            $html .= '<div class="form-group" style="margin-left: 10px; margin-top: 5px;"><input type="button" class="btn btn-default btn-outline" value="Excel 导出" id="excelExportData" > </div>';
            $html .= '<div class="form-group" style="margin-left: 10px; margin-top: 5px;"><a class="btn btn-default btn-outline" onclick="javascript:location.reload();" title="刷新">
